...
add_action('template_redirect', function () {
    if (empty($GLOBALS['dk_elementor_buf_started'])) {
        ob_start();
        $GLOBALS['dk_elementor_buf_started'] = true;
    }
}, 0);

add_action('wp_footer', function () {
    if (!function_exists('dk_elementor_is_cookie_set')) return;
    if (empty($GLOBALS['dk_elementor_buf_started'])) return;

    $html = ob_get_contents();
    if ($html === false || $html === '') return;

    dk_elementor_is_cookie_set($html);
}, 1);

function dk_elementor_is_cookie_set($html) {

    $opt = get_option("Elementor_page");
    if (!is_array($opt) || empty($opt)) return;

    // global cookie option (new structure)
    $cookie_on  = isset($opt['elementor_cookie_option']) && (string)$opt['elementor_cookie_option'] === "1";
    $cookieDays = isset($opt['elementor_cookie_option_days']) ? (int)$opt['elementor_cookie_option_days'] : 7;

    if (!$cookie_on) return;

    // must have at least one elementor form in page
    if (stripos($html, 'elementor-form') === false) return;

    // OPTIONAL: only set cookie if the page contains at least one configured Elementor node_id
    // (keys look like "Form name.af2bbed")
    $nodeIds = [];
    foreach ($opt as $k => $v) {
        if (!is_string($k) || !is_array($v)) continue; // skip globals
        $pos = strrpos($k, '.');
        if ($pos === false) continue;
        $node = substr($k, $pos + 1);
        if ($node !== '') $nodeIds[] = $node;
    }
    $nodeIds = array_values(array_unique($nodeIds));

    // if we have configured node IDs, require at least one to be present in markup
    if ($nodeIds) {
        $found = false;

        // Elementor markup commonly includes data-id="af2bbed"
        foreach ($nodeIds as $nid) {
            if (stripos($html, 'data-id="' . $nid . '"') !== false || stripos($html, "data-id='{$nid}'") !== false) {
                $found = true;
                break;
            }
        }

        // if not found, bail (prevents cookie set on pages with random forms)
        if (!$found) return;
    }

    // define constants used elsewhere (your helper)
    dk_checked_defined_constants('dk_cookie_unique_time', md5(microtime(true) . mt_rand()));
    dk_checked_defined_constants('dk_cookie_days_persistence', $cookieDays);

    // set cookie once (client side)
    add_action('wp_footer', function () { ?>
        <script id="duplicate-killer-elementor-form" type="text/javascript">
            (function () {
                ...
                // only set if Elementor form exists
                if (document.querySelector('form.elementor-form')) {
                    if (!getCookie('dk_form_cookie')) {
                        var date = new Date();
                        date.setDate(date.getDate() + <?php echo (int)dk_cookie_days_persistence; ?>);
                        document.cookie =
                            "dk_form_cookie=<?php echo esc_js(dk_cookie_unique_time); ?>" +
